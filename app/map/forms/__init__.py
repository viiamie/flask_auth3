...
class add_location_form(FlaskForm):
    title = TextAreaField('Location Name', [validators.DataRequired(), validators.length(max=300)], description="Name of the location")
    longitude = TextAreaField('Longitude', [validators.DataRequired(), validators.length(max=300)], description="Longitude of the location")
    latitude = TextAreaField('Latitude', [validators.DataRequired(), validators.length(max=300)], description="Latitude of the location")
    population = IntegerField('Population', [validators.DataRequired()], description="Population of the location")
    submit = SubmitField()